...
///
pub type FChannelSubscriberCallback<TEventArgs, THandlerException> = Rc<
    dyn Fn(
        FExecutionContext,
        FChannelSubscriberCallbackEventArgs<TEventArgs>,
...
mod tests {
    ...
    pub struct MyChannelConsumer<TEventArgs> {
        handlers: RefCell<Vec<FChannelSubscriberCallback<TEventArgs, FException>>>,
    }
                ...
                Box::pin(async move {
                    *counter_ref.borrow_mut() += 1;
                    if let FChannelSubscriberCallbackEventArgs::Event(event_args) = event {
                        data_collect.borrow_mut().push(event_args.data().clone());
                    }
