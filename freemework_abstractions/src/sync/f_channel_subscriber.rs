...
pub enum FChannelSubscriberCallbackEventArgs<TEventArgs> {
...
///
pub type FChannelSubscriberCallback<TEventArgs, THandlerException> = Arc<
    dyn Fn(FExecutionContext, FChannelSubscriberCallbackEventArgs<TEventArgs>) -> BoxFuture<'static, Result<(), THandlerException>>
        + Send
        + Sync,
>;
...
///
pub trait FChannelSubscriber<TEventArgs> {
    fn add_handler(&self, cb: &FChannelSubscriberCallback<TEventArgs, FException>);
    fn remove_handler(&self, cb: &FChannelSubscriberCallback<TEventArgs, FException>);
}
...
mod tests {
    ...
    pub struct MyChannelConsumer<TEventArgs> {
        handlers: Mutex<Vec<FChannelSubscriberCallback<TEventArgs, FException>>>,
    }
    impl<TEventArgs> FChannelSubscriber<TEventArgs> for MyChannelConsumer<TEventArgs> {
        fn add_handler(&self, cb: &FChannelSubscriberCallback<TEventArgs, FException>) {
    ...
    impl<TEventArgs: Clone + Send + Sync + 'static> MyChannelConsumer<TEventArgs> {
        pub async fn notify(
            ...
            for handler in handlers {
                ...
                handler(execution_context.clone(), FChannelSubscriberCallbackEventArgs::Event(my_event)).await?;
            }
            Ok(())
        }
    }
    ...
    async fn test_async_subscribe() {
        ...
        let handler1: FChannelSubscriberCallback<i32, FException> = {
            ...
            Arc::new(move |execution_context, event| {
                ...
                Box::pin(async move {
                    *counter_ref.lock().unwrap() += 1;
                    if let FChannelSubscriberCallbackEventArgs::Event(event_args) = event {
                        data_collect.lock().unwrap().push(event_args.data().clone());
                    }
        ...
        let handler2: FChannelSubscriberCallback<i32, FException> = {
